...
Next: 显式暂存本轮治理文件，复查 staged diff 与 Git 发布门；若发布失败保留本地 final 并记录 `final-not-pushed`。每日 daemon 仍只在 Docker 内运行，模型优先级保持 Astra low → Sol max → Terra max。
